# add other features for selecting table column ranges
...
df=pd.read_excel(DATA_PATH.joinpath('../data/s.txt'),sep=' ')
df=np.round(df,3)
...
layout = html.Div([
    dcc.Markdown('''
        ## 1184 quaternary alloy compositions screened using the hierarchical model; original publication accessible here. This project was sponsored by NSF'''),
    html.Br(),
    ...
    html.Div([
        ...
        dcc.Input(id='alloyID2', value=0, type='number',placeholder="Alloy ID", debounce=True,min=0, max=1183, step=1)
        ]),
    html.Br(),
...
@app.callback(
    ...
    Output(component_id='prediction2', component_property='children'),
    Output(component_id='PD2', component_property='figure'),
    Input(component_id='alloyID2', component_property='value')
    )
...
def update_output(input_id):
    ...
    print(p)
    for i in range(1,13):
        t=sorted(glob('../../../../../mnt/c/Users/adity/Documents/Refractories/TCP/TCP-data/TC-data'+str(i)+'/TC-data/*'))
        temp=[int(j.split('/')[-1].split('.')[0]) for j in t]
        if p in temp:
            folder=i
            break
    print(i)
    a=[]
    with open('../../../../../mnt/c/Users/adity/Documents/Refractories/TCP/TCP-data/TC-data'+str(i)+'/TC-data/'+str(p)+'.txt') as f:
        for line in f:
            a.append(line.split())
